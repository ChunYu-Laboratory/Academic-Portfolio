...
% message signal
m=[ones(1,round(t0/(3*ts))),-2*ones(1,round(t0/(3*ts))),zeros(1,round(t0/(3*ts))+1)];
...
env=abs(hilbert(u));                      	% Find the envelope.
...
env_r=abs(hilbert(r));                     	% envelope, when noise is present
